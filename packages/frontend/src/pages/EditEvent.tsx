...
export default function EditEvent() {
  ...
  useEffect(() => {
    ...
    api<EventData>(`/events/${eventId}`, { token: token ?? undefined })
      .then(data => {
        ...
        if (data.startTime) {
          const [time, ampm] = data.startTime.split(" ");
          const [h, m] = time.split(":");
          setStartHours(h);
          setStartMinutes(m);
          setStartAmpm(ampm || "am");
        }
        if (data.endTime) {
          const [time, ampm] = data.endTime.split(" ");
          const [h, m] = time.split(":");
          setEndHours(h);
          setEndMinutes(m);
          setEndAmpm(ampm || "am");
  ...
  const validateDate = (value: string) => {
    ...
    if (!value) { setDateError("Date is required"); return false; }
    if (!regex.test(value)) { setDateError("Please enter a valid date"); return false; }
    const [mm, dd, yyyy] = value.split("/").map(Number);
    const parsed = new Date(yyyy, mm - 1, dd);
    if (parsed.getMonth() !== mm - 1 || parsed.getDate() !== dd) { setDateError("Invalid date"); return false; }
    setDateError("");
    return true;
  };
 
  const validateTime = (sH: string, sM: string, sA: string, eH: string, eM: string, eA: string) => {
    if (!sH || !sM || !eH || !eM) return true;
    ...
    if (end <= start) { setTimeError("End time must be after start time"); return false; }
    setTimeError("");
    return true;
  };
  ...
  const handleSave = () => {
    ...
    .then(() => navigate(`/modules/${moduleId}/calendar/${eventId}`))
    .catch(() => {});
  };
 
  if (loading) return <p className="text-center text-[#8f9bb3] text-sm mt-8">Loading...</p>;
    ...
    >
      ...
      <div className="relative flex items-center justify-center px-6 pt-8 pb-4">
        <button
          ...
          onClick={() => navigate(`/modules/${moduleId}/calendar/${eventId}`)}
        >‹</button>
        <h1 className="text-4xl font-normal text-black">Edit Event</h1>
      </div>
      ...
      <div className="flex flex-col gap-6 px-6 pt-2">
        ...
        <div className="flex flex-col gap-3">
          ...
          {!allDay && (
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <div className="w-[60px] h-[42px] bg-white rounded-xl shadow flex items-center justify-center">
                    <input type="text" value={startHours} onChange={(e) => { setStartHours(e.target.value); validateTime(e.target.value, startMinutes, startAmpm, endHours, endMinutes, endAmpm); }} placeholder="HH" maxLength={2} className="w-full text-center text-[#c7c7c7] text-[15px] bg-transparent outline-none border-none" />
                  </div>
                  <span className="text-[#c7c7c7]">:</span>
                  <div className="w-[60px] h-[42px] bg-white rounded-xl shadow flex items-center justify-center">
                    <input type="text" value={startMinutes} onChange={(e) => { setStartMinutes(e.target.value); validateTime(startHours, e.target.value, startAmpm, endHours, endMinutes, endAmpm); }} placeholder="MM" maxLength={2} className="w-full text-center text-[#c7c7c7] text-[15px] bg-transparent outline-none border-none" />
                  </div>
                  <div className="w-[60px] h-[42px] bg-white rounded-xl shadow flex items-center justify-center relative">
                    <select value={startAmpm} onChange={(e) => { setStartAmpm(e.target.value); validateTime(startHours, startMinutes, e.target.value, endHours, endMinutes, endAmpm); }} className="w-full text-center text-[#c7c7c7] text-[15px] bg-transparent outline-none border-none cursor-pointer appearance-none">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <div className="w-[60px] h-[42px] bg-white rounded-xl shadow flex items-center justify-center">
                    <input type="text" value={endHours} onChange={(e) => { setEndHours(e.target.value); validateTime(startHours, startMinutes, startAmpm, e.target.value, endMinutes, endAmpm); }} placeholder="HH" maxLength={2} className="w-full text-center text-[#c7c7c7] text-[15px] bg-transparent outline-none border-none" />
                  </div>
                  <span className="text-[#c7c7c7]">:</span>
                  <div className="w-[60px] h-[42px] bg-white rounded-xl shadow flex items-center justify-center">
                    <input type="text" value={endMinutes} onChange={(e) => { setEndMinutes(e.target.value); validateTime(startHours, startMinutes, startAmpm, endHours, e.target.value, endAmpm); }} placeholder="MM" maxLength={2} className="w-full text-center text-[#c7c7c7] text-[15px] bg-transparent outline-none border-none" />
                  </div>
                  <div className="w-[60px] h-[42px] bg-white rounded-xl shadow flex items-center justify-center relative">
                    <select value={endAmpm} onChange={(e) => { setEndAmpm(e.target.value); validateTime(startHours, startMinutes, startAmpm, endHours, endMinutes, e.target.value); }} className="w-full text-center text-[#c7c7c7] text-[15px] bg-transparent outline-none border-none cursor-pointer appearance-none">
                      <option value="am">am</option>
                      <option value="pm">pm</option>
                    </select>
                    <span className="absolute right-2 text-[#c7c7c7] text-[8px] pointer-events-none">▼</span>
          ...
          {timeError && <p className="text-red-400 text-xs px-1">{timeError}</p>}
          {allDay && <p className="text-[#8f9bb3] text-xs px-1">This event will last the entire day.</p>}
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="bg-white rounded-2xl shadow px-4 py-3 h-[93px]">
            <textarea value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Enter Title Here" className="w-full h-full resize-none text-[15px] text-[#222b45] placeholder-[#c7c7c7] bg-transparent outline-none border-none" />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="bg-white rounded-2xl shadow px-4 py-3 h-[180px]">
            <textarea value={details} onChange={(e) => setDetails(e.target.value)} placeholder="Enter Details Here" className="w-full h-full resize-none text-[15px] text-[#222b45] placeholder-[#c7c7c7] bg-transparent outline-none border-none" />
          </div>
        </div>
