...
export default function CalendarScreenUser() {
  ...
  const currentModuleId = moduleId ?? id;

  useEffect(() => {
    if (!currentModuleId) return;

    api<CalendarEvent[]>(`/events/module/${currentModuleId}`, {
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-between px-16 py-2">
            <button
              onClick={prevMonth}
              className="text-2xl text-[#222b45] w-8 h-8 flex items-center justify-center"
            >
              ‹
            </button>
          ...
          <div className="grid grid-cols-7 px-4 mt-2">
            {DAYS_OF_WEEK.map(d => (
              <div key={d} className="text-center text-[#8f9bb3] text-[13px] py-2">
                {d}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 px-4">
            {buildCells().map((cell, i) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-9 h-9 flex flex-col items-center justify-center rounded-[10px] ${
          ...
          <div className="px-6 space-y-3">
            {!selectedDate ? (
              <p className="text-center text-[#8f9bb3] text-sm mt-4">
              ...
              selectedEvents.map((ev, i) => (
                <div
                  key={i}
                  onClick={() => navigate(`/modules/${currentModuleId}/calendar/${ev.id}`)}
                ...
                >
                  <div className="flex gap-3 items-start">
                    ...
                    <div>
                      ...
                      <div className="text-[#222b45] font-semibold text-base tracking-wide">
                        {ev.title}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-0.5">
                    {[0, 1, 2].map(d => (
                      <div key={d} className="w-1 h-1 bg-[#8f9bb3] rounded-full" />
                    ))}
                  </div>
                </div>
              ))
